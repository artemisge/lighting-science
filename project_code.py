...
from luxpy.toolboxes.spectro import jeti
import luxpy as lx
...
def make_luminance_plots(array_measured_spectra):
    ...
    for i in range(len(array_measured_spectra)):
        ...
        for j in range(len(array_measured_spectra[0])):
            array_luminance_i.append(lx.spd_to_power(array_measured_spectra[i][j], ptype = 'pu'))
        ...
        array_normalized_luminance_i = lx.spd_normalize(array_luminance_i, norm_type = 'max')
